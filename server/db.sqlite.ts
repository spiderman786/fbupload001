...
function migrateScrapeAndSchedule() {
  ...
  if (!pasNames.has('next_publish_at')) {
    db.exec(`ALTER TABLE page_automation_settings ADD COLUMN next_publish_at TEXT`)
    db.exec(`CREATE INDEX IF NOT EXISTS idx_pas_next_publish ON page_automation_settings(next_publish_at)`)
  }
}
